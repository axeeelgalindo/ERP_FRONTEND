...
export default function ModalBase({ open, title, onClose, children, footer, hideHeader }) {
  useEffect(() => {
    function onKey(e) {
      ...
      <div className="w-full max-w-4xl rounded-2xl bg-white shadow-xl border border-slate-200 overflow-hidden">
        {!hideHeader && (
          <div className="p-4 border-b border-slate-100 flex items-start justify-between gap-3">
            ...
            <button
              className="h-9 w-9 rounded-lg border border-slate-200 hover:bg-slate-50 cursor-pointer"
              onClick={onClose}
              type="button"
              title="Cerrar"
            >
