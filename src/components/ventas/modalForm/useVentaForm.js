...
export default function useVentaForm({
  ...
  const validateForm = () => {
    ...
    if (isOverQuoteLimit) {
      return `El total del costeo (${formatMoney(preview.total, moneda)}) supera el total de la cotización vinculada (${formatMoney(selectedOrdenVenta.total, selectedOrdenVenta.moneda || moneda)}).`;
    }
